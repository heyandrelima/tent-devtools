...
function defaultMappingsPath(): string {
  ...
  if (platform === 'darwin') {
    return path.join(homedir(), 'Library', 'Application Support', APP_NAME, 'mappings.json');
  }
  if (platform === 'win32') {
...
async function loadMappings(): Promise<void> {
  try {
    ...
    for (const item of data as Mapping[]) {
      ...
      const name = String(item.appName || '').trim().toLowerCase();
      if (!name) continue;
      next.add(`${name}.test`);
    }
    rootZones = next;
    // eslint-disable-next-line no-console
    console.log(`${LOG_PREFIX} Loaded ${rootZones.size} root zone(s) from ${MAPPINGS_PATH}`);
  } catch (err) {
    rootZones = new Set();
    const msg = err instanceof Error ? err.message : String(err);
    // eslint-disable-next-line no-console
    console.warn(`${LOG_PREFIX} Failed to load mappings (${msg}). Using empty set.`);
  }
}
...
export async function main(): Promise<unknown> {
  ...
  const server = UDPServer({
    ...
    handle: (request: any, send: (packet: any) => void) => {
      try {
        const [question] = (request.questions || []) as Array<{ name: string; type: string | number }>;
        const name = (question?.name || '').toLowerCase();
        const qtypeRaw = question?.type;
        const qtype = typeof qtypeRaw === 'number' ? String(qtypeRaw) : (qtypeRaw || '').toString().toUpperCase();
        ...
        if (match) {
          if (qtype === 'AAAA') {
            answers.push({ name, type: 'AAAA', class: 'IN', ttl: 1, address: '::1' });
          } else if (qtype === 'A' || qtype === 'ANY' || qtype === '1') {
            answers.push({ name, type: 'A', class: 'IN', ttl: 1, address: '127.0.0.1' });
          }
        }
        ...
        try {
          send({ id: request.id, type: 'response', questions: request.questions, answers: [] });
        } catch {
          // ignore
        }
      }
  ...
  server.on('listening', () => {
    ...
    if (rootZones.size === 0) {
      // eslint-disable-next-line no-console
      console.log(`${LOG_PREFIX} No mappings loaded. Add mappings in the Tend UI to enable responses.`);
    }
  });

  server.on('close', () => console.log(`${LOG_PREFIX} DNS stopped`));
  server.on('error', (e: unknown) => console.error(`${LOG_PREFIX} DNS error:`, e));
